...
- **Proactive usage poller now probes api_key accounts** (remote-dev-hobx, Phase 2 of epic remote-dev-3b3l) — the `UsageEndpointPoller` previously loaded only the subscription OAuth token and returned null for `api_key` profiles, so the (already implemented + unit-tested) api_key rate-limit-header parsing in `anthropic-usage-adapter` was never exercised by a real poll. The poller now resolves the profile's raw Anthropic API key through the **same precedence as the model-key proxy's `resolveProviderKey`** — the profile's encrypted `profileSecretsConfig` (`ANTHROPIC_API_KEY`, decrypted server-side by the existing `fetchProfileSecrets`; decryption is never re-implemented) first, then the `ANTHROPIC_API_KEY` env fallback — and hands it to the adapter, which probes with `x-api-key` and folds the worst-case rate/credit "org" dimension onto the result's 5h slot. When no key is resolvable the poll is a safe no-op (null, no probe). The key is the request credential only — never logged, persisted, or returned (only `accountKind` + numeric usage flow out). Still **OFF by default** behind `RDV_CLAUDE_USAGE_POLL_ENABLED=1`. (remote-dev-hobx)
