...
- **Instance user authorization now seeds at boot from `AUTHORIZED_USERS`; the supervisor seed Job is removed** (remote-dev-sb98) — a freshly provisioned k8s instance previously had no automated path to authorize users: the supervisor's first-boot seed Job (`buildSeedJob`) was never dispatched (a Job can't co-mount the instance's RWO PVC while the StatefulSet pod holds it) and the manual `bun run db:seed` CLI isn't shipped in the standalone image, so operators hand-inserted rows. Instead, the supervisor now injects `AUTHORIZED_USERS` (from the instance's `authorizedEmails`) as a **plain** env on the instance StatefulSet, and the app seeds its `authorized_users` table from it at boot — a new idempotent step (`src/db/boot-seed.ts`, `onConflictDoNothing`) wired into `src/instrumentation.ts` right after migrate-on-boot, running on **both** backends (the table exists by then: SQLite via the entrypoint's pre-app schema bootstrap, PostgreSQL via migrate-on-boot). The reconciler reads the emails from the `instance_seed` row and passes them through `provisionInstance` → `buildStatefulSet`. The step is **non-fatal** (an already-seeded instance still boots if it hiccups; failures warn loudly + structured) and logs only a **count** at info (the emails appear only at debug). `buildSeedJob` and its tests are deleted; the manual `src/db/seed.ts` CLI is unchanged (local/dev use only). The steady-state reconciler only patches replicas/image/PVC (never re-applies the full StatefulSet spec) and `createNamespacedStatefulSet` is 409-idempotent, so an **existing** instance is not disturbed and never loops; it picks up the env (and retro-seeds, idempotently) the next time its StatefulSet is recreated. (remote-dev-sb98)
