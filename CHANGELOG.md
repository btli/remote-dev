...
- Multi-instance OIDC login still bounced back to `/login` even after the `RDV_BASE_PATH` inlining fix above, because **the proxy's `AUTH_SECRET` (and `AUTH_URL`/`RDV_INSTANCE_SLUG`) were also empty at runtime**. In Next standalone output the proxy/middleware bundle does not reliably receive the container's *runtime* environment (vercel/next.js#53367) — and unlike `RDV_BASE_PATH`, `AUTH_SECRET` is a per-instance secret minted at provision time, so it **cannot** be baked into the build-once/shared image. With an empty secret the proxy's `getToken()` could not decrypt *any* valid session cookie → returned null → redirected every authenticated page back to `/login`, while the login page (rendered by the main Node server, which *does* have the runtime env) saw the real session and redirected back, an infinite loop. The empty `AUTH_URL` compounded it: `isSecureScheme()` returned false, so the proxy computed the **unprefixed** cookie name (`rdv-<slug>-session-token`) instead of the `__Secure-`-prefixed one the HTTPS auth handler wrote — a salt mismatch that rejects the cookie even with the right secret. Fixed with a tiny **runtime-env bridge** (`src/lib/runtime-env.ts`): `instrumentation.ts` `register()` runs once at startup on the Node runtime (same process + `globalThis` that later loads the Node-middleware proxy, verified empirically) and snapshots `AUTH_SECRET`/`AUTH_URL`/`NEXTAUTH_URL`/`RDV_INSTANCE_SLUG` into `globalThis.__RDV_RUNTIME_ENV`; the proxy, `getSessionCookieName()` (`auth-cookies.ts`), and `INSTANCE_SLUG` (`base-path.ts`) now read these via `runtimeEnv()`, which prefers a populated `process.env` and falls back to the snapshot only when the live value is empty. No secret is ever written to disk or baked into an image layer (the snapshot lives only in process memory). Single-server (`RDV_BASE_PATH=""`) is byte-identical: there the proxy shares the live `process.env`, so the fallback is never consulted. (remote-dev-jvcx)
