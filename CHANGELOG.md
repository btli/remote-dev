...
- Mobile session switcher: tapping the session title (down-caret) in the session view now opens an expandable bottom sheet listing every signed-in server/workspace and its active sessions; picking one switches the active server if needed and jumps to that session. (remote-dev-lxco)
