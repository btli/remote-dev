...
- **PKCE-style anti-hijack `state` on the mobile auth deep link** (remote-dev-gkuo) — the mobile login round-trips credentials (CF Access token / NextAuth session cookie / API key) back to the Flutter app via a `remotedev://auth/callback` **custom-scheme** deep link, which any app on the device could also register and intercept to steal those credentials. The app now generates a single-use, cryptographically-random **256-bit** `state` nonce (`Random.secure()`, base64url, no padding) for each login attempt, appends it to the `/auth/mobile-callback?state=…` URL it opens, and **rejects any callback whose echoed `state` is missing or does not exactly match** the one it issued — so a hijacked, forged, or replayed callback can neither complete the login nor inject its own credentials. Both server callback resolvers echo the nonce **unchanged on the deep link for every credential mode** (instance: `src/lib/mobile-callback.ts` + `src/app/auth/mobile-callback/page.tsx`; supervisor: `apps/supervisor/src/lib/mobile-callback.ts` + its page), and the pages thread it **inside the `?callbackUrl=`** so it survives the OIDC/login round-trip. The nonce is cleared after each attempt (single-use, so replays after completion are rejected). Strictly additive — the existing CF/OIDC/API-key multi-mode flow is unchanged; an older app that sends no `state` still works (the server simply omits the echo). **Version coupling:** because the instance and supervisor servers both echo `state`, an updated app always talks to an updated server, so a missing echo cannot occur for a legitimate callback — hence the app enforces the match **strictly**. (remote-dev-gkuo)
