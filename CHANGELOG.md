...
- **Instance image (`@libsql` native binding):** fixed a Docker build failure (`cannot copy to non-directory: .../@libsql/client`) in the slug-aware instance image. The runtime stage now copies only the leaf `@libsql/linux-x64-gnu` binding into the Next.js standalone tree's existing `@libsql/` directory, instead of overlaying the whole staged scope onto its symlinked entries. Provisioned k3s instances no longer CrashLoop with `Cannot find module '@libsql/linux-x64-gnu'`.
