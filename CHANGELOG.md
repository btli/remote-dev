...
- Instance image Dockerfile: provisioned k8s instances CrashLooped at startup (`exit 1`) — now past basePath materialization, the **terminal server** died immediately with `Cannot find module '@libsql/linux-x64-gnu'` (Require stack: `/app/dist-terminal/index.js`). The base app's DB layer (`src/db/index.ts`) uses `@libsql/client/node`, which conditionally `require`s the host's platform-native binding at runtime (on the linux-x64 image, `@libsql/linux-x64-gnu`). The slim runtime stage staged only `node-pty` + `better-sqlite3` into `/app/node_modules`, and the Next.js standalone trace does not capture that conditional native require — so the binding was absent and the Node-run terminal server (CWD=`/app`) could not resolve it → `exit 1` → CrashLoop. (Distinct from the supervisor's earlier `@libsql/isomorphic-*` bun-export-condition issue: instances run under **node** per `docker/entrypoint.sh`, so this is the Node platform binding.) Fix mirrors the existing native-module staging: the build stage now `cp -RL node_modules/@libsql /opt/native/libsql-scope` (the whole scope incl. the native binding + `client`/`core`/`isomorphic-*`), the runtime stage `COPY`s it into `/app/node_modules/@libsql` (resolvable from `/app` by both the Next.js app and `dist-terminal`), and the native-ABI smoke test now also `require`s `@libsql/client/node` so a future packaging regression fails the build instead of CrashLooping at startup. (remote-dev-qy7t)
