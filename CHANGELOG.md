...
- Instance image is now a real **dev environment**, not a stripped runtime: it bakes all **5 agent CLIs** onto the system PATH (`/usr/local/bin`) — `claude` (`@anthropic-ai/claude-code`), `codex` (`@openai/codex-cli`), `gemini` (`@google/gemini-cli`), `opencode`, and `agy` (Antigravity, via its curl installer) — using the project's documented install methods, so agents launched as the PTY shell command resolve immediately. `sudo` is installed and the `rdv` user gets passwordless sudo (a deliberate single-tenant homelab choice) so agents can `sudo apt-get install …` their own toolchains; `apt` is kept fully functional (keyring + sources intact, index pruned for size — run `sudo apt-get update` first). A build-time `command -v` smoke check fails the build if any CLI is missing, and the entrypoint kicks off a **non-blocking, best-effort background auto-update** (`npm update -g …` + antigravity self-update) after the servers start so long-lived instances keep agents current without ever stalling or failing boot. (remote-dev-fmcq)
