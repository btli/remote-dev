...
/**
 * Tests for POST /api/instances/:id/start — an exact behavioral alias of
 * /resume. Same operator role, same owner-scoping, SAME canonical audit action
 * "resume" / status `ready`.
 */
...
import { POST } from "@/app/api/instances/[id]/start/route";

function req(id: string): [Request, { params: Promise<{ id: string }> }] {
  return [
    new Request(`https://sup.example.com/api/instances/${id}/start`, { method: "POST" }),
...
describe("POST /api/instances/:id/start (alias of resume)", () => {
  it("suspended → 202, marked ready, canonical audit action 'resume'", async () => {
