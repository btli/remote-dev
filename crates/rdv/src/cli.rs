...
pub enum HookAction {
    ...
    ///
    /// When called without arguments, reads JSON from stdin (Claude Code hook format).
    /// When called with tool_name, uses command-line arguments (manual usage).
    PostToolUse {
        /// Tool name (e.g., "Bash", "Read", "Edit", "Write"). If omitted, reads JSON from stdin.
