...
pub async fn execute(cmd: HookCommand, config: &Config) -> Result<()> {
    match cmd.action {
        ...
        HookAction::PostToolUse {
            ...
            output,
            path,
        } => post_tool_use(&tool_name, input.as_deref(), output.as_deref(), path.as_deref()).await,
        HookAction::Stop {
            agent,
            reason,
            path,
...
async fn post_tool_use(
    ...
    let skip_tools = ["Read", "Glob", "Grep", "LS", "WebSearch"];
    if skip_tools.iter().any(|t| tool_name.eq_ignore_ascii_case(t)) {
        return Ok(());
    }
    ...
    let content = build_tool_observation(tool_name, input, output);
    ...
    if let Some(out) = output {
        if contains_error_indicators(out) {
            let gotcha_content = format!(
                ...
                input.map(|i| truncate_content(i, 200)).unwrap_or_default()
            );
            ...
            let _ = store_memory(
                ...
                "working",
                "gotcha",
                Some(&format!("Error: {} failure", tool_name)),
                folder_id.as_deref(),
            );
        }
    }
...
fn build_tool_observation(tool_name: &str, input: Option<&str>, output: Option<&str>) -> String {
    match tool_name.to_lowercase().as_str() {
        "bash" => {
            let cmd = input.unwrap_or("unknown command");
        ...
        "task" => {
            let desc = input.map(|i| truncate_content(i, 150)).unwrap_or("task".to_string());
            format!("Spawned subagent: {}", desc)
        }
        "todowrite" => {
            // Extract task summary from TodoWrite
            if let Some(inp) = input {
                format!("Updated task list: {}", truncate_content(inp, 200))
            } else {
...
fn get_folder_id_for_path(project_path: &str) -> Option<String> {
    ...
    // List all folders and find by path or name (avoids user_id requirement)
    // Since we're single-user, we can search without user_id scoping
    let folders = db.list_all_folders().ok()?;
    folders.into_iter().find(|f| {
        f.path.as_deref() == Some(project_path) || f.name == folder_name
    }).map(|f| f.id)
}
