...
pub async fn create_session(
    ...
    // Detect project metadata if project_path is provided
    // This is quick detection for session creation - comprehensive analysis
    // is available via MCP tools (project_metadata_detect)
    if let Some(ref path) = req.project_path {
        let project_path = std::path::Path::new(path);
        if project_path.exists() {
            match project::detect_project(project_path) {
                Ok(info) => {
                    info!(
                        "Detected project info for {}: language={:?}, framework={:?}, package_manager={:?}",
                        path,
                ...
                Err(e) => {
                    ...
                    warn!("Failed to detect project info for {}: {}", path, e);
                }
            }
        }
    }
