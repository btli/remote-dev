...
/**
 * Phase 3: build the node-preferences inheritance chain for a project or
 * group node, in ancestor-first order (most-specific last). For a project
 * node, the chain starts with the project's own prefs, then walks up each
 * ancestor group. For a group node, the chain walks up the group ancestry.
 *
 * Returns the raw `fields` payload from each `NodePreferences` record so
 * Phase 4 resolvers can layer them onto the existing ResolvedPreferences
 * output without coupling to the domain types here.
 *
 * NOTE: this function is server-only — it lazily imports the DI container to
 * avoid pulling better-sqlite3 / drizzle into browser bundles that import
 * the rest of this module.
 */
export async function buildNodeAncestry(
  node: { id: string; type: "group" | "project" },
  userId: string
): Promise<Array<{ ref: { type: "group" | "project"; id: string }; fields: Record<string, unknown> }>> {
  if (typeof window !== "undefined") {
    throw new Error("buildNodeAncestry is server-only");
  }

  // Lazy, server-only import to keep this module safe on the client.
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { container } = require("@/infrastructure/container") as typeof import("@/infrastructure/container");
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { NodeRef } = require("@/domain/value-objects/NodeRef") as typeof import("@/domain/value-objects/NodeRef");

  const chain: Array<{ ref: { type: "group" | "project"; id: string }; fields: Record<string, unknown> }> = [];
  ...
  if (node.type === "project") {
    ...
    if (selfPrefs) {
      chain.push({ ref: { type: "project", id: project.id }, fields: selfPrefs.fields });
    }

    const ancestors = await container.projectGroupRepository.listAncestry(project.groupId);
    for (const g of ancestors) {
      const p = await container.nodePreferencesRepository.findForNode(
        NodeRef.group(g.id),
        userId
      );
      if (p) chain.push({ ref: { type: "group", id: g.id }, fields: p.fields });
    }
  } else {
