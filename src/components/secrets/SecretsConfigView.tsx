...
/**
 * SecretsConfigView - Per-project secrets configuration UI.
 *
 * Extracted from the legacy `SecretsConfigModal` so the same markup can be
 * embedded directly in a terminal-type tab (no Dialog wrapper). Mirrors the
 * original overview/configure tabs; when an `initialFolderId` is supplied the
 * configure tab is pre-selected for that project.
 */
...
export interface SecretsConfigViewProps {
  /** Pre-select a project's configuration when opening. */
  initialFolderId?: string | null;
  /**
   * Optional close handler. When omitted (e.g. rendered inside a persistent
   * terminal tab) the Close button is hidden.
   */
  onClose?: () => void;
}

export function SecretsConfigView({ initialFolderId, onClose }: SecretsConfigViewProps) {
  ...
  // Active tab
  const [activeTab, setActiveTab] = useState<"overview" | "configure">(
    initialFolderId ? "configure" : "overview"
  );

  // Secrets live on leaf projects — groups don't own their own credentials.
  ...
  // Seed the form state once from `initialFolderId` or the active project.
  // We intentionally do NOT re-seed on every folderConfigs change — the user
  // may be mid-edit and shouldn't be clobbered by a background refresh.
  useEffect(() => {
  ...
  }, [initialFolderId, activeProject.folderId]);
  ...
  return (
    <div className="flex flex-col h-full min-h-0 overflow-hidden">
      ...
      >
        <TabsList className="w-full bg-muted/50 flex-shrink-0 mt-4">
