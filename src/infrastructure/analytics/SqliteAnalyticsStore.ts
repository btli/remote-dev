/**
 * SqliteAnalyticsStore - better-sqlite3 implementation of the AnalyticsStore
 * port.
 *
 * This is the original `litellm-analytics-service` logic, extracted verbatim
 * into a class so it can sit behind the AnalyticsStore port alongside the new
 * Postgres implementation. Behavior is byte-for-byte identical to the prior
 * service:
 *   - `recordBatch` writes synchronously inside a single better-sqlite3
 *     transaction, using the in-batch `sampleCache` for latency coherence.
 *   - read methods run synchronous queries; they return `Promise.resolve(...)`
 *     only to satisfy the (now async-read) port. No I/O is deferred.
...
export class SqliteAnalyticsStore implements AnalyticsStore {
  ...
  getTimeSeries(opts: TimeSeriesOptions): Promise<TimeSeriesPoint[]> {
    ...
    if (opts.granularity === "hourly") {
      ...
      return Promise.resolve(db.prepare(sql).all(...params) as TimeSeriesPoint[]);
    }

    // Daily or weekly — read from litellm_daily_agg
    const dateExpr =
      opts.granularity === "weekly" ? `strftime('%Y-W%W', date)` : "date";
   ...
   */
  pruneOldLogs(retentionDays = 90): Promise<{ deletedCount: number }> {
    const db = getAnalyticsDatabase();
