...
import { pgTable, text, integer, bigint, boolean, timestamp, jsonb, index, uniqueIndex, primaryKey } from "drizzle-orm/pg-core";
...
export const terminalSessions = pgTable(
  "terminal_session",
  {
    ...
    claudeAccountId: text("claude_account_id").references(() => claudeAccounts.id, { onDelete: "set null" }),
    ...
    agentExitNotificationAt: timestamp("agent_exit_notification_at", { withTimezone: true, mode: "date" }),
    agentRestartCount: integer("agent_restart_count").default(0),
    agentActivityStatus: text("agent_activity_status"),
    agentActivityStatusAt: bigint("agent_activity_status_at", { mode: "number" }),
    agentActivityOrder: bigint("agent_activity_order", { mode: "number" }),
...
export const agentStatusDeliveries = pgTable(
...
export const claudeAccounts = pgTable(
  "claude_account",
  {
    ...
    usageOauthAccessEncrypted: text("usage_oauth_access_encrypted"),
    usageOauthRefreshEncrypted: text("usage_oauth_refresh_encrypted"),
    usageOauthExpiresAt: timestamp("usage_oauth_expires_at", { withTimezone: true, mode: "date" }),
    usageOauthScopes: text("usage_oauth_scopes"),
  ...
  (table) => [
    ...
    index("claude_account_user_email_idx").on(table.userId, table.emailAddress),
  ]
);
...
export const claudeUsageLimitStates = pgTable(
  ...
  {
    accountId: text("account_id").primaryKey().references(() => claudeAccounts.id, { onDelete: "cascade" }),
    userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
...
export const claudeProfilePoolMembers = pgTable(
  ...
  (table) => [
    uniqueIndex("claude_pool_member_pool_account_unique").on(table.poolId, table.accountId),
    index("claude_pool_member_pool_priority_idx").on(table.poolId, table.priority),
    index("claude_pool_member_account_idx").on(table.accountId),
  ]
);
