...
export class Folder {
   ...
   * Validates that the move doesn't create a cycle (caller must provide all folders).
   */
  ...
  /** Check if this folder belongs to specified user */
  belongsTo(userId: string): boolean {
    return this.props.userId === userId;
  }

  /** Check if this folder is a child of the specified folder */
  isChildOf(folderId: string): boolean {
    return this.props.parentId === folderId;
  }

  /**
   * Check if moving to newParentId would create a cycle.
   * Walks up the parent chain from newParentId to see if it reaches this folder.
   */
